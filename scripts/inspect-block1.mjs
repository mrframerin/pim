...
function children(parentFull){
  const oe=findTagEnd(parentFull,0);
  const tagName=/^<([a-zA-Z][\w:-]*)/.exec(parentFull)[1];
  const inner=parentFull.slice(oe+1, parentFull.lastIndexOf("</"+tagName));
  const out=[];let cur=0;
  while(cur<inner.length){const n=inner.indexOf("<",cur);if(n<0)break;if(inner.startsWith("<!--",n)){const ce=inner.indexOf("-->",n+4);cur=ce<0?inner.length:ce+3;continue;}const el=sliceEl(inner,n);if(!el)break;out.push(el);cur=el.end;}
  return out;
}

// get main, then block index 1
const ms=html.indexOf("<main");const mainEl=sliceEl(html,ms);
const top=children(mainEl.full);
const block1=top[1];
console.log("BLOCK[1] tag", block1.tag, "len", block1.full.length);
console.log("=== direct children of block[1] ===");
for(const [i,el] of children(block1.full).entries()){
  const cls=(/class="([^"]*)"/.exec(el.open)||[])[1]||"";
  const h=/<h[1-4][^>]*>([\s\S]*?)<\/h[1-4]>/.exec(el.full||"");
  const heading=h?h[1].replace(/<[^>]+>/g,"").trim().slice(0,55):"";
  console.log(` [1.${i}] <${el.tag}> len=${el.full.length} class="${cls.slice(0,60)}" ${heading?"H:"+heading:""}`);
}
